...
final class MonitoringViewModel: ObservableObject {
    ...
    private func bind() {
        ...
        observeUseCase.journeyPlanStream()
            .receive(on: DispatchQueue.main)
            .sink { [weak self] items in
                self?.journeyPlanItems = items
            }
            .store(in: &cancellables)
    }
